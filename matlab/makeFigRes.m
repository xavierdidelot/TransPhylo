function makeFigRes(record,names)
...
g=biograph.bggui(bg);
fig=figure;subplot(3,2,[3 5]);a=gca;
...
for i=2:n+1
    ...
    ids{i}=sprintf('%s [%.2f;%.2f]',names{i-1},tinf2(fi,i,1)/tinf2(fi,i,2)-000,trec(i-1)-000);
...
g=biograph.bggui(bg);
fig2=fig;figure(fig2);subplot(3,2,[4 6]);a=gca;
copyobj(allchild(g.biograph.hgAxes),a);%Copy biograph viewer to current axes
