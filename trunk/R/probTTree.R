...
probTTree = function(ttree,R,pi,w.shape,w.scale)  {
  ...
  for (i in (1:n)) { 
    ...
    {prob<-prob+log(pi);
    prob <- prob + dgamma((ttree[i,2]-ttree[i,1]),shape=w.shape,scale=w.scale,log=TRUE)} 
    offspring <- which( ttree[ ,3] == i ) 
    d0 <- length(offspring)
    if (is.na(pd0[d0+1])) {
      pd0[d0+1]=d0*log(R)+R*(p0-1)-d0*log(R*p0)+pgamma(R*p0,d0,log=T)#log(1-d0*gamma_inc(d0,R*p0)/gamma(1+d0))
    }
    prob <- prob + pd0[d0+1]
