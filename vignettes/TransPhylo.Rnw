...
A pathogen has an effective within-host population size of $N_e=100$ and a generation time $g=1$ day, so that $N_e g=100/365$ year. The offspring distribution is negative binomial with parameters (5,0.5) so that the basic reproduction number is $R=5$. Both the generation time and the sampling time are Gamma distributed with parameters (10,0.1) which has a mean of 1 year. The density of sampling is $\pi=0.25$. The following commands specify these parameters:
<<>>=
neg=100/365
off.r=5
off.p=0.5
...
record<-inferTTree(ptree,mcmcIterations=100,w.shape=w.shape,w.scale=w.scale,
                   ...
                   updateOff.r=T,updatePi=T,updateNeg=T,updateOff.p=F,dateT=2008)
@
