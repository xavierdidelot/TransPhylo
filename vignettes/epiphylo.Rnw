...
<<>>=
simu <- simulateOutbreak(R=1,neg=100/365,ninf=1,pi=0.5)
@
...
full<-makeFullTreeFromPTree(ptree,dateLastSample=max(simu[,1]))
plotBothTree(full)
@
...
<<results=hide>>=
record<-inferTTree(ptree,dateLastSample=max(simu[,1]),mcmcIterations=100)
@
