...
<<>>=
simu <- simulateOutbreak(R=1,neg=100/365,ninf=20,pi=0.1,w.shape=20,w.scale=0.1)
@
...
plotTTree(ttree,w.shape=20,w.scale=0.1)
@
...
<<results=hide>>=
record<-inferTTree(ptree,mcmcIterations=1000,w.shape=20,w.scale=0.1,updateNeg=F,updateR=F)
@
