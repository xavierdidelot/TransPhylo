...
inferTTree = function(ptree,w.shape=2,w.scale=1,ws.shape=w.shape,ws.scale=w.scale,mcmcIterations=1000,thining=1,startNeg=100/365,startOff.r=1,startOff.p=0.5,startPi=0.5,updateNeg=T,updateOff.r=T,updateOff.p=T,updatePi=T,startFulltree=NA,updateTTree=TRUE,optiStart=T,datePresent=Inf) {
  ...
  off.p <- startOff.p
  pi <- startPi
  if (is.na(sum(startFulltree))) fulltree <- makeFullTreeFromPTree(ptree,ifelse(optiStart,off.r,NA),off.p,neg,pi,w.shape,w.scale,ws.shape,ws.scale,datePresent)#Starting point 
  else fulltree<-startFulltree
  ttree <- ttreeFromFullTree(fulltree)
  record <- vector('list',mcmcIterations/thining)
  pTTree <- probTTree(ttree,off.r,off.p,pi,w.shape,w.scale,ws.shape,ws.scale,datePresent) 
  ...
  for (i in 1:mcmcIterations) {#Main MCMC loop
    if (i%%thining == 0) {
      ...
      record[[i/thining]]$ws.shape <- ws.shape
      record[[i/thining]]$ws.scale <- ws.scale
    ...
    pTTree2 <- probTTree(ttree2,off.r,off.p,pi,w.shape,w.scale,ws.shape,ws.scale,datePresent) 
    ...
    if (updateOff.r) {
      ...
      pTTree2 <- probTTree(ttree,off.r2,off.p,pi,w.shape,w.scale,ws.shape,ws.scale,datePresent) 
    ...
    if (updateOff.p) {
      ...
      pTTree2 <- probTTree(ttree,off.r,off.p2,pi,w.shape,w.scale,ws.shape,ws.scale,datePresent) 
      ...
      if (pi2>1) pi2=2-pi2
      pTTree2 <- probTTree(ttree,off.r,off.p,pi2,w.shape,w.scale,ws.shape,ws.scale,datePresent) 
