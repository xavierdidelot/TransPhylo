...
inferTTree = function(ptree,w.shape=2,w.scale=1,mcmcIterations=1000,startNeg=100/365,startOff.r=1,startOff.p=0.5,startPi=0.5,updateNeg=TRUE,updateOff.r=TRUE,updateOff.p=TRUE,updatePi=TRUE,testing=FALSE) {
  if (testing) {
  ...
  pTTree <- probTTree(ttreeFromFullTree(fulltree),off.r,off.p,pi,w.shape,w.scale) 
  ...
  for (i in 1:mcmcIterations) {#Main MCMC loop
    ...
    fulltree2 <- prop$tree
    pTTree2 <- probTTree(ttreeFromFullTree(fulltree2),off.r,off.p,pi,w.shape,w.scale) 
    ...
    if (updateOff.r) {
      ...
      pTTree2 <- probTTree(ttreeFromFullTree(fulltree),off.r2,off.p,pi,w.shape,w.scale) 
    ...
    if (updateOff.p) {
      ...
      pTTree2 <- probTTree(ttreeFromFullTree(fulltree),off.r,off.p2,pi,w.shape,w.scale) 
      ...
      if (pi2>1) pi2=2-pi2
      pTTree2 <- probTTree(ttreeFromFullTree(fulltree),off.r,off.p,pi2,w.shape,w.scale) 
